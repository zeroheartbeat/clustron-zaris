...
(Zaris) store in a continuously updating console view.

## Description

`Watch-ZrStoreMetrics` provides a **real‑time monitoring view** of
...
``` powershell
Watch-ZrStoreMetrics -StoreName <string> [-RefreshSec <int>] [-Servers <string[]>]
```
...
``` powershell
Watch-ZrStoreMetrics -StoreName OrdersStore -RefreshSec 3
```
...
If neither `-Servers` nor an active `Connect-ZrManager` session exists,
