...
`Add-DictusInstance` expands a running or configured store by adding
...
-   `-Servers` parameter
-   Active `Connect-DictusManager` session context
...
``` powershell
Add-DictusInstance -StoreName <string> -InstanceName <string> -ClustronPort <int> -ClientPort <int>
```

### Add multiple instances

``` powershell
Add-DictusInstance -StoreName <string> -Nodes <InstanceDefinition[]>
```
...
Add-DictusInstance -StoreName OrdersStore -Nodes $nodes
```
...
The cmdlet returns a **DictusAdminResult** object describing the outcome of
the operation.
