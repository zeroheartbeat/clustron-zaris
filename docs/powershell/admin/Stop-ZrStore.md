...
Stops one or more instances of a **Clustron Distributed Key-Value (Zaris)
store**.

## Description

`Stop-ZrStore` stops running instances of a Clustron store on one or
...
The cmdlet outputs one **ZarisAdminResult** per instance stop operation.
...
``` powershell
Stop-ZrStore -Name <string>
```

### Stop a specific instance

``` powershell
Stop-ZrStore -Name <string> -InstanceName <string>
```
...
``` powershell
Stop-ZrStore -Name OrdersStore -InstanceName orders-node-1
```
...
``` powershell
Stop-ZrStore -Name OrdersStore -InstanceName orders-node-1 -Force
```
...
The cmdlet returns one **ZarisAdminResult** object per instance stop
operation.
