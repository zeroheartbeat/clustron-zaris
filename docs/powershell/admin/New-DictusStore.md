...
Creates a new **Clustron Distributed Key‑Value (Dictus) store** on one or
more management servers.

## Description

`New-DictusStore` provisions a new distributed store in the Clustron
cluster.\
...
-   `-Servers` parameter
-   The active `Connect-DictusManager` session context
...
``` powershell
New-DictusStore -Name <string> -InstanceName <string> -ClustronPort <int> -ClientPort <int>
```

### Multi Instance

``` powershell
New-DictusStore -Name <string> -Instances <InstanceDefinition[]>
```
...
New-DictusStore -Name OrdersStore -Instances $instances
```
...
The cmdlet returns a **DictusAdminResult** object describing the result of
