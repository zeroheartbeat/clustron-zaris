...
Starts one or more instances of a **Clustron Distributed Key‑Value (Dictus)
store**.

## Description

`Start-DictusStore` starts instances of an existing Clustron store on one
...
The cmdlet outputs one **DictusAdminResult** per instance start operation.
...
``` powershell
Start-DictusStore -Name <string>
```

### Start a specific instance

``` powershell
Start-DictusStore -Name <string> -InstanceName <string>
```
...
``` powershell
Start-DictusStore -Name OrdersStore -InstanceName orders-node-1
```
...
The cmdlet returns one **DictusAdminResult** object per instance start
operation.
