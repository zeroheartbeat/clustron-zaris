...
Retrieves information about **Clustron Distributed Key-Value (Zaris)
stores** and their runtime status.

## Description

`Get-ZrStore` queries the Clustron manager API and returns a summary of
...
-   `-Servers` parameter
-   Active `Connect-ZrManager` session
-   Fallback resolution from `ZarisCmdletBase`
...
``` powershell
Get-ZrStore -Name <string>
```

Optional parameters inherited from `ZarisCmdletBase`:
...
If omitted, the cmdlet uses the current `Connect-ZrManager` session
context.
...
``` powershell
Get-ZrStore -Servers 10.0.0.11,10.0.0.12
```
