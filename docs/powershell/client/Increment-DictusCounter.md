...
``` powershell
Increment-DictusCounter [-Key] <string> [-Delta <long>]
```
...
`Increment-DictusCounter` increases the value of a numeric counter stored
in a Clustron store.
...
``` powershell
Increment-DictusCounter -Key orders:processed
```
...
``` powershell
Increment-DictusCounter -Key orders:processed -Delta 5
```
...
``` powershell
Increment-DictusCounter -Key orders:processed -Delta 10
```
