...
Tests whether a **key exists** in a Clustron Distributed Key-Value (Dictus)
store.
...
``` powershell
Test-DictusItem [-Key] <string> [-Detailed]
```
...
`Test-DictusItem` checks whether a key exists in the connected Clustron
store.
...
``` powershell
Test-DictusItem -Key session:user123
```
...
``` powershell
Test-DictusItem -Key session:user123 -Detailed
```
