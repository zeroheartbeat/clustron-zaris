...
``` powershell
Watch-ZrPrefix [-Prefix] <string> [-IncludeSnapshot]
```
...
`Watch-ZrPrefix` subscribes to change events for all keys that share a
common prefix.
...
``` powershell
Watch-ZrPrefix -Prefix config: -IncludeSnapshot
```
