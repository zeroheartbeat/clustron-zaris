...
``` powershell
Set-DictusItemExpiration [-Key] <string> [-TimeToLive] <TimeSpan>
```
...
`Set-DictusItemExpiration` sets or updates the expiration time for an
existing item.
...
``` powershell
Set-DictusItemExpiration -Key session:user123 -TimeToLive (New-TimeSpan -Minutes 10)
```
...
``` powershell
Set-DictusItemExpiration -Key cache:item1 -TimeToLive (New-TimeSpan -Seconds 60)
```
...
``` powershell
Set-DictusItemExpiration -Key cache:product:100 -TimeToLive (New-TimeSpan -Seconds 30)
```
...
-   Use `Refresh-DictusItem` to extend the existing TTL instead.
