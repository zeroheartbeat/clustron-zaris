...
``` powershell
Decrement-ZrCounter [-Key] <string> [-Delta <long>]
```
...
`Decrement-ZrCounter` decreases the value of a numeric counter stored
in a Clustron store.
...
``` powershell
Decrement-ZrCounter -Key inventory:item-100
```
...
``` powershell
Decrement-ZrCounter -Key inventory:item-100 -Delta 5
```
...
``` powershell
Decrement-ZrCounter -Key inventory:item-100 -Delta 10
```
