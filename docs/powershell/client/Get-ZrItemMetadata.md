...
`Get-ZrItemMetadata` retrieves metadata associated with a key without
focusing on the stored value itself.

The command returns internal metadata maintained by the Zaris cluster
including:
...
``` powershell
Get-ZrItemMetadata -Key order:1001
```
...
``` powershell
Get-ZrItemMetadata -Key session:abc
```
