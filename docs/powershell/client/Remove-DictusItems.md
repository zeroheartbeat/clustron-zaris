...
Deletes multiple items from a **Clustron Distributed Key‑Value (Dictus)
...
``` powershell
Remove-DictusItems [-Keys] <string[]>
```
...
`Remove-DictusItems` deletes multiple keys from a Clustron store in a
single batch operation.

Bulk deletion is significantly more efficient than issuing multiple
`Remove-DictusItem` commands individually because the request is executed
...
``` powershell
Remove-DictusItems -Keys "user:1","user:2","user:3"
```
...
``` powershell
Remove-DictusItems -Keys "session:1","session:2","session:3"
```
...
``` powershell
"cache:1","cache:2","cache:3" | Remove-DictusItems
```
