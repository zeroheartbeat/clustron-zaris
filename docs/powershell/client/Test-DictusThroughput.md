# Test-DictusThroughput

## Synopsis

Measures **operation throughput (OPS/sec)** of a Clustron Distributed
Key-Value (Dictus) store under concurrent load.
...
``` powershell
Test-DictusThroughput -StoreName <string> -DurationSec <int> -Concurrency <int> [-ObjectSizeBytes <int>]
```
...
`Test-DictusThroughput` runs a workload benchmark against a Clustron store
...
``` powershell
Test-DictusThroughput -StoreName TestStore -DurationSec 30 -Concurrency 16
```
...
``` powershell
Test-DictusThroughput -StoreName TestStore -DurationSec 60 -Concurrency 32
```
...
``` powershell
Test-DictusThroughput -StoreName TestStore -DurationSec 30 -Concurrency 64
```
...
``` powershell
Test-DictusThroughput -StoreName TestStore -DurationSec 30 -Concurrency 32 -ObjectSizeBytes 512
```
...
``` powershell
Test-DictusThroughput -StoreName TestStore -DurationSec 60 -Concurrency 64
```
...
``` powershell
Test-DictusThroughput -StoreName TestStore -DurationSec 30 -Concurrency 32 -ObjectSizeBytes 1024
```
