...
Revokes an existing **lease** in a Clustron Distributed Key-Value (Zaris)
store.
...
``` powershell
Revoke-ZrLease [-LeaseId] <LeaseId>
```
...
`Revoke-ZrLease` immediately invalidates a previously granted lease.
