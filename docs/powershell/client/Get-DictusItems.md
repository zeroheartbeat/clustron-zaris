...
Retrieves multiple items from a **Clustron Distributed Key-Value (Dictus)
...
`Get-DictusItems` retrieves values for multiple keys from a Clustron store
using a single bulk request.

This cmdlet is optimized for high-performance retrieval when working
with many keys. Instead of issuing multiple `Get-DictusItem` calls, this
...
``` powershell
Get-DictusItems -Keys "user:1","user:2","user:3"
```
...
``` powershell
Get-DictusItems -Keys "order:1001","order:1002","order:1003"
```
...
``` powershell
"order:1001","order:1002","order:1003" | Get-DictusItems
```
...
-   Bulk retrieval is significantly faster than multiple `Get-DictusItem`
    calls.
