...
``` powershell
Set-DictusItem [-Key] <string> [-Value] <object> [-ExpireAfterSec <int>] [-LeaseId <LeaseId>]
```
...
`Set-DictusItem` stores a value under a specified key in a Clustron store.
...
``` powershell
Set-DictusItem -Key user:100 -Value "John"
```
...
``` powershell
Set-DictusItem -Key session:100 -Value "abc123" -ExpireAfterSec 300
```
...
``` powershell
Set-DictusItem -Key leader -Value node1 -LeaseId $lease
```
...
``` powershell
Set-DictusItem -Key order:1001 -Value "processed"
```
...
``` powershell
Set-DictusItem -Key session:abc -Value "token" -ExpireAfterSec 600
```
