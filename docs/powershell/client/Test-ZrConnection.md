...
Tests connectivity to a **Clustron Distributed Key-Value (Zaris) store**.
...
``` powershell
Test-ZrConnection [-StoreName <string>] [-Endpoints <string[]>] [-TimeoutSec <int>]
```
...
`Test-ZrConnection` verifies whether a connection to a Clustron Zaris
...
``` powershell
Test-ZrConnection -StoreName SessionStore
```
...
``` powershell
Test-ZrConnection -StoreName SessionStore -Endpoints "127.0.0.1:7070"
```
...
``` powershell
Test-ZrConnection -StoreName SessionStore -Endpoints "node1:7070" -TimeoutSec 10
```
...
``` powershell
Test-ZrConnection -StoreName TestStore -Endpoints "node1:7070"
```
...
``` powershell
Test-ZrConnection -StoreName TestStore -Endpoints "node1:7070" -TimeoutSec 15
```
...
-   If a connection already exists via `Connect-ZrStore`, it will be
    reused.
