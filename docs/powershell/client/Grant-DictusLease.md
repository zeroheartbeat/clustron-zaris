...
Creates a **lease** in a Clustron Distributed Key-Value (Dictus) store.
...
``` powershell
Grant-DictusLease [-TimeToLive] <TimeSpan>
```
...
`Grant-DictusLease` creates a lease with a specified time-to-live (TTL).
...
Once granted, the lease ID can be used with operations such as:
    ...
    Set-DictusItem -LeaseId <LeaseId>
...
``` powershell
Grant-DictusLease -TimeToLive (New-TimeSpan -Seconds 30)
```
...
``` powershell
Grant-DictusLease -TimeToLive (New-TimeSpan -Minutes 5)
```
