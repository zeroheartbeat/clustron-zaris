...
``` powershell
Refresh-ZrLease [-LeaseId] <LeaseId>
```
...
`Refresh-ZrLease` sends a **keep‑alive request** for an existing lease
...
Calling `Refresh-ZrLease` resets the lease timer and keeps the lease
active.
